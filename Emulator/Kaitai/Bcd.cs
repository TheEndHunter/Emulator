    ...
    public partial class Bcd : KaitaiStruct
    {
        ...
        private void Read()
        {
            ...
            for (var i = 0; i < NumDigits; i++)
            {
                switch (BitsPerDigit)
                {
                    case 4:
                        {
                            _digits.Add((int)m_io.ReadBitsIntBe(4));
                            break;
                        }
                    case 8:
                        {
        ...
        public int AsIntLe
        {
            get
            {
                if (f_asIntLe)
                    return _asIntLe;
                _asIntLe = Digits[0] + (NumDigits < 2 ? 0 : ((Digits[1] * 10) + (NumDigits < 3 ? 0 : ((Digits[2] * 100) + (NumDigits < 4 ? 0 : ((Digits[3] * 1000) + (NumDigits < 5 ? 0 : ((Digits[4] * 10000) + (NumDigits < 6 ? 0 : ((Digits[5] * 100000) + (NumDigits < 7 ? 0 : ((Digits[6] * 1000000) + (NumDigits < 8 ? 0 : (Digits[7] * 10000000))))))))))))));
                f_asIntLe = true;
                return _asIntLe;
            }
        }
        ...
        public int AsIntBe
        {
            get
            {
                if (f_asIntBe)
                    return _asIntBe;
                _asIntBe = (Digits[LastIdx] + (NumDigits < 2 ? 0 : ((Digits[(LastIdx - 1)] * 10) + (NumDigits < 3 ? 0 : ((Digits[(LastIdx - 2)] * 100) + (NumDigits < 4 ? 0 : ((Digits[(LastIdx - 3)] * 1000) + (NumDigits < 5 ? 0 : ((Digits[(LastIdx - 4)] * 10000) + (NumDigits < 6 ? 0 : ((Digits[(LastIdx - 5)] * 100000) + (NumDigits < 7 ? 0 : ((Digits[(LastIdx - 6)] * 1000000) + (NumDigits < 8 ? 0 : (Digits[(LastIdx - 7)] * 10000000)))))))))))))));
                f_asIntBe = true;
                return _asIntBe;
            }
        }
        private List<int> _digits;
        private readonly byte _numDigits;
        private readonly byte _bitsPerDigit;
        private readonly bool _isLe;
        private readonly Bcd m_root;
        ...
        public Bcd Root { get { return m_root; } }
        public KaitaiStruct Parent { get { return m_parent; } }
    }
}
